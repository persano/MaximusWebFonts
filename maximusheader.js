  ...
  //console.log(jsondata)
    ...
    function logono2() {if ("{(GBP_CUSTOMER)}" != -1) {
        ...
        cat.addEventListener('mouseover', () => {
            ...
            if(catsSecundarias[categorias.indexOf(cat)].children[0].classList.contains(none)){
                ...
                //catsSecundarias[categorias.indexOf(cat)].children[0].classList.remove(none)
			  	for (var i = 0; i < catsSecundarias[categorias.indexOf(cat)].children.length; i++) {
					catsSecundarias[categorias.indexOf(cat)].children[i].classList.remove(none);
				}
  		  ...
  		  if(suggestions && userData.length > 0){
  			 /* busquedas = suggestions.filter(data => {
  			  ...
  			  suggestions.forEach(sugg => {
  				  mayoresCatsRepetidas.push({'cat': `${sugg.cat}`, 'cat_id': `${sugg.cat_id}`});
  				  mayoresSubCatsRepetidas.push({'subcat': `${sugg.subcat}`, 'subcat_id': `${sugg.subcat_id}`});
  			  })
  			  ...
  			  mayoresSubCatsRepetidas.sort((a, b) => a.subcat_id.localeCompare(b.subcat_id));

  			  //remuevo las categorias repetidas
  			  mayoresCatsRepetidas = mayoresCatsRepetidas.filter((cat, index) => cat != mayoresCatsRepetidas[index - 1]).splice(0, 1)
  			  mayoresSubCatsRepetidas = mayoresSubCatsRepetidas.filter((subcat, index) => subcat != mayoresSubCatsRepetidas[index - 1]).splice(0, 1)

  			  //creo el html con las categorias limitadas
  			  suggestions = suggestions.splice(0, 5).map((data) => {
  				  return data =  `<div class="auto-search">
  									  <a href='https://www.maximus.com.ar/DETALLE/${data.nomProd.split(' ').join('-')}/ITEM_ID=${data.ID}/maximus.aspx' class="auto-link"><img class="imgSuggestion" src="https://www.maximus.com.ar/Temp/App_WebSite/App_PictureFiles/Items/${data.codeImg}_100.jpg" width="50" height="50"><strong>${data.nomProd}</strong></a>
  								  </div>`
  			  }).join('')
  			  ...
  			  mayoresSubCatsRepetidas = mayoresSubCatsRepetidas.map(subcat => {
  			  ...
  			  let totalHTML = suggestions + ' <hr class="thin-line container"> ' + mayoresCatsRepetidas + ' <hr class="thin-line container"> ' + mayoresSubCatsRepetidas
...
//alert(JSON.stringify(suggestions[1]));
            ...
            if(suggestions && userData.length > 0){
              /*busquedas = suggestions.filter(data => {
                ...
                //creo el html con las categorias limitadas
                suggestions = suggestions.splice(0, 3).map((data) => {
                    return data =  `<div class="auto-search">
                                        <a href='https://www.maximus.com.ar/DETALLE/${data.nomProd.split(' ').join('-')}/ITEM_ID=${data.ID}/maximus.aspx' class="auto-link"><img class="imgSuggestion" src="https://www.maximus.com.ar/Temp/App_WebSite/App_PictureFiles/Items/${data.codeImg}_100.jpg" width="50" height="50"><strong class="auto-link-name">${data.nomProd}</strong></a>
                                    </div>`
                }).join('');
    ...
    const catsSecs = [...document.querySelectorAll('.apertura-cat-principal-static')];
    catsSecs.map(cat=>{
      cat.addEventListener('mouseover', () => {
        agregarNone()
      })
